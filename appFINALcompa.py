...
def parse_dates(df: pd.DataFrame) -> pd.DataFrame:
    for col in ["Alojamiento", "Fecha alta", "Fecha entrada", "Fecha salida", "Precio"]:
...
def load_excel_from_blobs(file_blobs: List[tuple[str, bytes]]) -> pd.DataFrame:
    ...
    Se cachea por contenido; así evitamos recargar si no cambian.
    """
    frames = []
...
# --- Gestor de archivos a nivel global (persisten en session_state) ---
with st.sidebar:
...
# =============================
# MODO 1: Consulta normal (+ comparación año anterior)
# =============================
if mode == "Consulta normal":
    ...
    if compare_normal:
        ...
        st.markdown("**Comparativa con año anterior** (corte y periodo desplazados -1 año)")
...
# =============================
# MODO 2: KPIs por meses (línea) + línea de comparación
# =============================
    ...
    with st.sidebar:
        ...
        inv_m = st.number_input("Sobrescribir inventario (nº alojamientos)", min_value=0, value=0, step=1, key="inv_months")
    ...
    if selected_months_m:
        ...
        for ym in selected_months_m:
            ...
            if compare_m:
                p_prev = p - 12  # mismo mes año anterior
        ...
        if not compare_m:
            st.line_chart(df_actual.set_index("Mes")[[metric_choice]], height=280)
            ...
            # Construir DataFrame para dos líneas
            plot_df = pd.DataFrame({
                "Mes": df_actual["Mes"],
                "Actual": df_actual[metric_choice.lower().replace(" (€)", "").replace(" ", "_") if metric_choice!="Ocupación %" else "ocupacion_pct"],
            })
            if not df_prev.empty:
                prev_col = metric_choice.lower().replace(" (€)", "").replace(" ", "_") if metric_choice!="Ocupación %" else "ocupacion_pct"
                plot_df["Año anterior"] = df_prev[prev_col].values
            plot_df = plot_df.set_index("Mes")
...
# =============================
# MODO 3: Evolución por fecha de corte + línea de comparación
# =============================
    ...
    with st.sidebar:
        ...
        inv_e = st.number_input("Sobrescribir inventario (nº alojamientos)", min_value=0, value=0, step=1, key="inv_evo")
    ...
    if run_evo:
        ...
        if cut_start_ts > cut_end_ts:
            ...
            for c in pd.date_range(cut_start_ts, cut_end_ts, freq="D"):
                ...
                rows_e.append({"Corte": c.date().isoformat(), **tot_c})
            df_evo = pd.DataFrame(rows_e)

            if df_evo.empty:
                st.info("No hay datos para el rango seleccionado.")
            else:
                # Preparar serie a plotear
                col_map = {
                    "Ocupación %": "ocupacion_pct",
                    "ADR (€)": "adr",
                    "RevPAR (€)": "revpar",
                }
                key_col = col_map[metric_choice_e]
                plot_df = pd.DataFrame({
                    "Paso": range(len(df_evo)),
                    "Actual": df_evo[key_col].values,
                }).set_index("Paso")

                # Serie de comparación (año anterior, mismo largo si es posible)
                if compare_e:
                    rows_prev = []
                    ...
                    for c in pd.date_range(cut_start_prev, cut_end_prev, freq="D"):
                        ...
                        rows_prev.append(tot_c2)
                    df_prev = pd.DataFrame(rows_prev)
                    length = min(len(plot_df), len(df_prev))
                    plot_df = plot_df.iloc[:length]
                    plot_df["Año anterior"] = df_prev[key_col].values[:length]
